...
import type { ExportIR, PluginSettings, SelectionMode } from './types'

/** UI → sandbox: begin extraction with the given mode and requestId */
export interface ExtractRequestHandler extends EventHandler {
  name: 'EXTRACT_REQUEST'
  /**
   * requestId is a monotonic counter generated by the UI. The sandbox echoes it
   * in every EXTRACT_COMPLETE / EXTRACT_ERROR response so the UI can discard
   * responses that belong to a superseded request.
   */
  handler: (mode: SelectionMode, requestId: number) => void
}
...
export interface ExtractCompleteHandler extends EventHandler {
  ...
  handler: (ir: ExportIR, requestId: number) => void
}
...
export interface ExtractErrorHandler extends EventHandler {
  name: 'EXTRACT_ERROR'
  handler: (message: string, requestId: number) => void
}

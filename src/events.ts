...
export interface ExtractRequestHandler extends EventHandler {
   ...
   * in every EXTRACT_COMPLETE / EXTRACT_ERROR / EXTRACT_PROGRESS response so the
   * UI can discard responses that belong to a superseded request.
   */
  handler: (mode: SelectionMode, options: ExtractOptions, requestId: number) => void
}
...
export interface ExtractProgressHandler extends EventHandler {
  ...
  handler: (progress: { processed: number; total: number; requestId: number }) => void
}

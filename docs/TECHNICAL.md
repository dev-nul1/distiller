...
`requestId` is a monotonic counter generated by the UI; the sandbox echoes it in every `EXTRACT_COMPLETE` and `EXTRACT_ERROR` response so stale responses from superseded requests can be discarded.
...
type RichRun = {
  ...
  underline?: boolean    // captured but not represented in any output format
...
type TableData = {
  ...
  hasHeader: boolean     // best-effort: true when the table has at least one row
}
...
- **Underline** is captured in the IR but has no output representation — text is preserved, the decoration is silently dropped.
